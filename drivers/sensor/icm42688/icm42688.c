...
int icm42688_configure(const struct device *dev, struct icm42688_cfg *cfg)
{
	...
	*/
	...
	if (cfg->fifo_en) {
		...
		/* TODO we have two interrupt lines, either can be used for watermark, choose 1 for now... */
		...
		/* Setup desired FIFO packet fields, maybe should base this on the other temp/accel/gyro en fields in cfg */
		res = icm42688_spi_single_write(&dev_cfg->spi, REG_FIFO_CONFIG1,
