				       ...
				       int8_t shift)
{
	...
	if (channel >= ARRAY_SIZE(sensor_channel_name)) {
		shell_info(sh, "channel idx=%d shift=%d value=%s%d.%06d", channel, shift,
			   is_negative ? "-" : "", numerator, denominator);
	} else {
		shell_info(sh, "channel idx=%d %s shift=%d value=%s%d.%06d", channel,
		...
		while (decoder->get_frame_count(buf, channel, channel_idx, &frame_count) == 0) {
			...
			while (decoder->decode(buf, channel, channel_idx, &fit, 1, decoded_buffer) >
				...
				case SENSOR_CHAN_ACCEL_XYZ: {
					...
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_ACCEL_X,
								   data->readings[0].x,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_ACCEL_Y,
								   data->readings[0].y,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_ACCEL_Z,
								   data->readings[0].z,
								   data->shift);
					break;
				}
				case SENSOR_CHAN_GYRO_XYZ: {
					...
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_GYRO_X,
								   data->readings[0].x,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_GYRO_Y,
								   data->readings[0].y,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_GYRO_Z,
								   data->readings[0].z,
								   data->shift);
					break;
				}
				case SENSOR_CHAN_MAGN_XYZ: {
					...
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_MAGN_X,
								   data->readings[0].x,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_MAGN_Y,
								   data->readings[0].y,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_MAGN_Z,
								   data->readings[0].z,
								   data->shift);
					break;
				}
				case SENSOR_CHAN_POS_DX: {
					...
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_POS_DX,
								   data->readings[0].x,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_POS_DY,
								   data->readings[0].y,
								   data->shift);
					sensor_shell_print_q_value(ctx->sh, SENSOR_CHAN_POS_DZ,
								   data->readings[0].z,
								   data->shift);
					break;
				}
				default: {
					...
					sensor_shell_print_q_value(ctx->sh, channel,
								   data->readings[0].value,
								   data->shift);
					break;
				}
				}
